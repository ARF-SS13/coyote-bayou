	...
	if(get_typing_indicator_pref() == GLOB.play_methods[PLAY_ANIMALCROSSING_TI])		//we are checking if they actually have this preference turned on 
		...
		for(var/i in 1 to counter)
			...
			spawn(timecounter)
				...
				playsound(playfrom, get_typing_indicator_sound(do_static), get_typing_indicator_volume(do_static), FALSE, SOUND_DISTANCE(0), SOUND_FALLOFF_EXPONENT, TI_frequency)
