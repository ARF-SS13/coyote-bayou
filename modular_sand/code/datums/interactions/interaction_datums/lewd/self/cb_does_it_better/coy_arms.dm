...
//Uses the 'temp_gender' system to determine if he or she, or they, should be used. Defined by characters GENDER, not body model.
	...
	if(user.a_intent == INTENT_HELP)	//The USER of the verbs intent, only the pitcher gets to determine how hard they throw.
		var/list/lewd = list(
			"gently mlems on one of <b>[partner]'s</b> fingers.",
			"softly licks on one of <b>[partner]'s</b> fingies",
			"teasingly blems one of <b>[partner]'s</b> digits",
			"teases <b>[partner]'s</b> fingers with [t_their] mouth"
		)
		message = pick(lewd)
	...
	//disarm intent should be used for being particularly playful with the interaction
		...
		var/list/lewd = list(
			"playfully laps at one of <b>[partner]'s</b> fingers.",
			"teasingly licks on <b>[partner]'s</b> fingies",
			"puts <b>[partner]'s</b> fingers against [t_their] tongue",
			"teases <b>[partner]'s</b> by dragging [t_their] tongue across [p_their] finger"
		)
		message = pick(lewd)
	...
	//grab intent should be used for playing rough, without actually being particulalry cruel or aggressive in said action
		...
		var/list/lewd = list(
			"really just licks all up on <b>[partner]'s</b> fingers.",
			"is really giving <b>[partner]'s</b> fingers a cleaning with their tongue",
			"drags [t_their] tongue around on <b>[partner]'s</b> fingers wetly",
			"leaves a bit of drool on <b>[partner]'s</b> fingers as they lick it like a lollipop"
		)
		message = pick(lewd)
	...
	if(user.a_intent == INTENT_HARM)
		var/list/lewd = list(
			"is drawing little circles around <b>[partner]'s</b> finger with thier tongue.",
			"is downright giving <b>[partner]</b> finger felatio with just their tongue",
			"might as well be trying to figure out how many licks it takes to get to the center of <b>[partner]'s</b> fingers",
			"sure is going to town on <b>[partner]'s</b> fingers with their licker"
		)
		message = pick(lewd)
