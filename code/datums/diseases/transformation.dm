	...
	switch(stage)
		if(1)
			if(ishuman(affected_mob) && affected_mob.dna)
				if(affected_mob.dna.species.id == SPECIES_SLIME || affected_mob.dna.species.id == SPECIES_SLIME_STARGAZER || affected_mob.dna.species.id == SPECIES_SLIME_LUMINESCENT)
					stage = 5
		if(3)
			if(ishuman(affected_mob))
				var/mob/living/carbon/human/human = affected_mob
				if(human.dna.species.id != SPECIES_SLIME && affected_mob.dna.species.id != SPECIES_SLIME_STARGAZER && affected_mob.dna.species.id != SPECIES_SLIME_LUMINESCENT)
