...
/datum/wound/blunt/receive_damage(wounding_type, wounding_dmg, wound_bonus)
	...
	if(limb.body_zone == BODY_ZONE_CHEST && victim.get_blood(TRUE) && prob(internal_bleeding_chance + wounding_dmg))
...
/datum/wound/blunt/get_scanner_description(mob/user)
	...
	else if(limb.body_zone == BODY_ZONE_CHEST && victim.get_blood(TRUE))
