...
/datum/dna/proc/generate_uni_identity()
	...
	if(ishuman(holder))
		...
		L[DNA_COLOR_ONE_BLOCK] = sanitize_hexcolor(features[FEATURE_COLOR_1], 6)
		L[DNA_COLOR_TWO_BLOCK] = sanitize_hexcolor(features[FEATURE_COLOR_2], 6)
		L[DNA_COLOR_THREE_BLOCK] = sanitize_hexcolor(features[FEATURE_COLOR_3], 6)
		L[DNA_MUTANTTAIL_BLOCK] = construct_block(SSfurry.tails.Find(features[MBP_TAIL]), SSfurry.tails.len)
		L[DNA_MUTANTEAR_BLOCK] = construct_block(SSfurry.ears.Find(features[MBP_EARS]), SSfurry.ears.len)
		L[DNA_MUTANTMARKING_BLOCK] = construct_block(SSfurry.body_markings.Find(features[MBP_BODY_MARKINGS]), SSfurry.body_markings.len)
		L[DNA_TAUR_BLOCK] = construct_block(SSfurry.taurs.Find(features[MBP_TAUR]), SSfurry.taurs.len)
...
/datum/dna/proc/update_ui_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_COLOR_ONE_BLOCK)
			sanitize_hexcolor(features[FEATURE_COLOR_1], 6)
		if(DNA_COLOR_TWO_BLOCK)
			sanitize_hexcolor(features[FEATURE_COLOR_2], 6)
		if(DNA_COLOR_THREE_BLOCK)
			sanitize_hexcolor(features[FEATURE_COLOR_3], 6)
		if(DNA_MUTANTTAIL_BLOCK)
			construct_block(SSfurry.tails.Find(features[MBP_TAIL]), SSfurry.tails.len)
		if(DNA_MUTANTEAR_BLOCK)
			construct_block(SSfurry.ears.Find(features[MBP_EARS]), SSfurry.ears.len)
		if(DNA_MUTANTMARKING_BLOCK)
			construct_block(SSfurry.body_markings.Find(features[MBP_BODY_MARKINGS]), SSfurry.body_markings.len)
		if(DNA_TAUR_BLOCK)
