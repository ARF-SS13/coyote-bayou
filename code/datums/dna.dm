...
/datum/dna/proc/generate_uni_identity()
	...
	if(ishuman(holder))
		...
		L[DNA_COLOR_ONE_BLOCK] = sanitize_hexcolor(features[MBP_COLOR1], 6)
		L[DNA_COLOR_TWO_BLOCK] = sanitize_hexcolor(features[MBP_COLOR2], 6)
		L[DNA_COLOR_THREE_BLOCK] = sanitize_hexcolor(features[MBP_COLOR3], 6)
		// if(!GLOB.mam_tails_list.len)
		// 	init_sprite_accessory_subtypes(/datum/sprite_accessory/tails/mam_tails, GLOB.mam_tails_list)
		L[DNA_MUTANTTAIL_BLOCK] = construct_block(GLOB.mam_tails_list.Find(features[MBP_TAIL]), GLOB.mam_tails_list.len)
		// if(!GLOB.mam_ears_list.len)
		// 	init_sprite_accessory_subtypes(/datum/sprite_accessory/ears/mam_ears, GLOB.mam_ears_list)
		L[DNA_MUTANTEAR_BLOCK] = construct_block(GLOB.mam_ears_list.Find(features[MBP_EARS]), GLOB.mam_ears_list.len)
		// if(!GLOB.mam_body_markings_list.len)
		// 	init_sprite_accessory_subtypes(/datum/sprite_accessory/mam_body_markings, GLOB.mam_body_markings_list)
		L[DNA_MUTANTMARKING_BLOCK] = construct_block(GLOB.mam_body_markings_list.Find(features[MBP_BODY_MARKINGS]), GLOB.mam_body_markings_list.len)
		// if(!GLOB.taur_list.len)
		// 	init_sprite_accessory_subtypes(/datum/sprite_accessory/taur, GLOB.taur_list)
		L[DNA_TAUR_BLOCK] = construct_block(GLOB.taur_list.Find(features[MBP_TAUR]), GLOB.taur_list.len)
...
/datum/dna/proc/update_ui_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_COLOR_ONE_BLOCK)
			sanitize_hexcolor(features[MBP_COLOR1], 6)
		if(DNA_COLOR_TWO_BLOCK)
			sanitize_hexcolor(features[MBP_COLOR2], 6)
		if(DNA_COLOR_THREE_BLOCK)
			sanitize_hexcolor(features[MBP_COLOR3], 6)
		if(DNA_MUTANTTAIL_BLOCK)
			construct_block(GLOB.mam_tails_list.Find(features[MBP_TAIL]), GLOB.mam_tails_list.len)
		if(DNA_MUTANTEAR_BLOCK)
			construct_block(GLOB.mam_ears_list.Find(features[MBP_EARS]), GLOB.mam_ears_list.len)
		if(DNA_MUTANTMARKING_BLOCK)
			construct_block(GLOB.mam_body_markings_list.Find(features[MBP_BODY_MARKINGS]), GLOB.mam_body_markings_list.len)
		if(DNA_TAUR_BLOCK)
			construct_block(GLOB.taur_list.Find(features[MBP_TAUR]), GLOB.taur_list.len)
			if(species.mutant_bodyparts[MBP_TAUR] && ishuman(holder))
