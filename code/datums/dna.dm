...
/datum/dna/proc/generate_uni_identity()
	...
	if(ishuman(holder))
		...
		L[DNA_MUTANTMARKING_BLOCK] = construct_block(GLOB.mam_body_markings_list.Find(features[MBP_MARKINGS_BODY]), GLOB.mam_body_markings_list.len)
...
/datum/dna/proc/update_ui_block(blocknumber)
	...
	switch(blocknumber)
		...
		if(DNA_MUTANTMARKING_BLOCK)
			construct_block(GLOB.mam_body_markings_list.Find(features[MBP_MARKINGS_BODY]), GLOB.mam_body_markings_list.len)
		if(DNA_TAUR_BLOCK)
