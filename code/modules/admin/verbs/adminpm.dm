...
/client/proc/cmd_admin_pm(whom, msg)
	...
	if(external)
		to_chat(src, span_notice("PM to-<b>Admins</b>: <span class='linkify'>[rawmsg]</span>"), confidential = TRUE)
		...
		if(recipient.holder && !badmin)
			if(holder)
				to_chat(recipient, span_danger("Admin PM from-<b>[key_name(src, recipient, 1)]</b>: <span class='linkify'>[keywordparsedmsg]</span>"), confidential = TRUE)
				to_chat(src, span_notice("Admin PM to-<b>[key_name(recipient, src, 1)]</b>: <span class='linkify'>[keywordparsedmsg]</span>"), confidential = TRUE)
				...
				to_chat(src, span_notice("PM to-<b>Admins</b>: <span class='linkify'>[msg]</span>"), confidential = TRUE)
				// SSblackbox.LogAhelp(current_ticket.id, "Reply", msg, recipient.ckey, src.ckey)

			//play the receiving admin the adminhelp sound (if they have them enabled)
			if(recipient.prefs.toggles & SOUND_ADMINHELP)
				SEND_SOUND(recipient, sound('sound/effects/adminnotification.ogg'))
			...
			if(holder)	//sender is an admin but recipient is not. Do BIG RED TEXT
				...
				to_chat(src, span_notice("Admin PM to-<b>[key_name(recipient, src, 1)]</b>: <span class='linkify'>[msg]</span>"), confidential = TRUE)
