...
/proc/send_pirate_threat()
	var/pirate_type = "pirate" //pick(PIRATES_ROGUES, PIRATES_SILVERSCALES, PIRATES_DUTCHMAN)
	...
	threat_msg.answer_callback = CALLBACK(GLOBAL_PROC_REF(pirates_answered), threat_msg, payoff, ship_name, initial_send_time, response_max_time, ship_template)
	addtimer(CALLBACK(GLOBAL_PROC_REF(spawn_pirates), threat_msg, ship_template, FALSE), response_max_time)
...
/obj/machinery/computer/piratepad_control/proc/send()
	...
	var/value = 0
	for(var/datum/export/E in ex.total_amount)
		var/export_text = E.total_printout(ex,notes = FALSE) //Don't want US Government messages, makes no sense here.
		if(!export_text)
			continue
