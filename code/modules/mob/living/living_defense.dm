...
/mob/living/bullet_act(obj/item/projectile/P, def_zone)
	...
	var/damage_threshold = max(run_armor_check("damage_threshold", P.flag, null, null, 0, null) - P.damage_threshold_penetration, 0)
	if(!P.nodamage)
		apply_damage(totaldamage, P.damage_type, def_zone, armor, wound_bonus = P.wound_bonus, bare_wound_bonus = P.bare_wound_bonus, sharpness = P.sharpness, damage_threshold = damage_threshold)
		if(P.dismemberment)
	...
	if(I)
		...
		if(!blocked)
			if(!nosell_hit)
				...
				var/damage_threshold = max(run_armor_check("damage_threshold", "melee", null, null, 0, null) - I.damage_threshold_penetration, 0)
				apply_damage(I.throwforce, dtype, impacting_zone, armor, sharpness=I.get_sharpness(), wound_bonus=(nosell_hit * CANT_WOUND), damage_threshold)
		else
			return 1
	else
