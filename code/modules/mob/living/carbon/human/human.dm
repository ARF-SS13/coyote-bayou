				...
				if(href_list["hud"] == "s")
					if(istype(H.glasses, /obj/item/clothing/glasses/hud/security) || istype(H.getorganslot(ORGAN_SLOT_HUD), /obj/item/organ/cyberimp/eyes/hud/security))
						...
						if(perpname)
							...
							if(R)
								if(href_list["status"])
									var/setcriminal = input(usr, "Specify a new criminal status for this person.", "Security HUD", R.fields["criminal"]) in list(ACCESSORY_NONE, "*Arrest*", "Incarcerated", "Paroled", "Discharged", "Cancel")
			...
			if(stat != DEAD)
				...
				for(var/X in bodyparts)
					...
					if(hal_screwyhud == SCREWYHUD_HEALTHY)
						icon_num = 0
					if(icon_num)
						hud_used.healthdoll.add_overlay(mutable_appearance('icons/mob/screen_gen.dmi', "[GLOB.bodypart_zone2legacy[BP.body_zone]][icon_num]"))
