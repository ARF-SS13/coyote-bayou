...
/mob/living/simple_animal/attack_ghost(mob/user, latejoinercalling)
	...
	if(!lazarused_by && living_player_count() < pop_required_to_jump_into)
		to_chat(user, span_warning("There needs to be at least [pop_required_to_jump_into] living players to hop in this! This check is bypassed if the mob has had a lazarus injector used on it though, or tamed. Which it hasn't (yet)."))
		return
