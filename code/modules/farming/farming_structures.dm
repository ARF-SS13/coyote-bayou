...
/obj/structure/fermenting_barrel/attackby(obj/item/I, mob/user, params)
	...
	if(istype(fruit))
		...
		for(var/obj/item in storage_contents)
			fruit = item
			if(!istype(fruit) || !fruit.can_distill || !SEND_SIGNAL(I, COMSIG_TRY_STORAGE_TAKE, fruit, src, FALSE, fruit.loc, current_equipped_slot))
				continue
			fruits += fruit
		if (length(fruits))
...
/obj/structure/reagent_dispensers/compostbin/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W, /obj/item/seeds) || istype(W, /obj/item/reagent_containers/food/snacks/grown))
		...
		for(var/obj/item/G in PB.contents)// This check can be less than thorough because the bag has already authenticated the contents, hopefully
			if(SEND_SIGNAL(PB, COMSIG_TRY_STORAGE_TAKE, G, src, FALSE, G.loc, current_equipped_slot))
	...
	if(istype(O, /obj/item/reagent_containers/food/snacks/grown/))
		...
		if(F.seed)
			if(user && !user.temporarilyRemoveItemFromInventory(O)) //couldn't drop the item
				return
			if (SEND_SIGNAL(O.loc, COMSIG_CONTAINS_STORAGE) && !SEND_SIGNAL(O.loc, COMSIG_TRY_STORAGE_TAKE, O, null, FALSE, O.loc, current_equipped_slot)) // couldn't remove from storage
				return
		...
		if(F.seed)
			if(user && !user.temporarilyRemoveItemFromInventory(O))
				return
			if (SEND_SIGNAL(O.loc, COMSIG_CONTAINS_STORAGE) && !SEND_SIGNAL(O.loc, COMSIG_TRY_STORAGE_TAKE, O, null, FALSE, O.loc, current_equipped_slot))
				return
			while(t_amount < t_max)
				var/obj/item/seeds/t_prod = F.seed.Copy()
				t_prod.forceMove(seedloc)
