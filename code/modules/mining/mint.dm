...
/obj/machinery/mineral/mint/proc/create_coins()
	...
	if(T)
		...
		if(QDELETED(bag_to_use) || (bag_to_use.loc != T) || !SEND_SIGNAL(bag_to_use, COMSIG_TRY_STORAGE_INSERT, O, null, TRUE, TRUE, O.loc, null)) //important to send the signal so we don't overfill the bag.
