...
/proc/load_custom_items_from_db(ckey)  //If the load_custom_items_from_db flag is enabled in config\config.txt, load all ckeys and their granted items into the global list defined above
	...
	while(custom_item_read.NextRow())
		custom_items[custom_item_read.item[1]] += list(unsanitizeSQL(custom_item_read.item[2]),unsanitizeSQL(custom_item_read.item[3]), unsanitizeSQL(custom_item_read.item[4]), custom_item_read.item[5], unsanitizeSQL(custom_item_read.item[6]))
