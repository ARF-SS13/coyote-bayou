...
/obj/item/melee/cultblade/dagger/proc/scribe_rune(mob/living/user)
	...
	if(ispath(rune_to_scribe, /obj/effect/rune/narsie))
		...
		for(var/B in spiral_range_turfs(1, user, 1))
			...
			shields += N
	user.visible_message(span_warning("[user] [user.get_blood(TRUE) ? "cuts open [user.p_their()] arm and begins writing in [user.p_their()] own blood":"begins sketching out a strange design"]!"), \
						span_cult("You [user.get_blood(TRUE) ? "slice open your arm and ":""]begin drawing a sigil of the Geometer."))
