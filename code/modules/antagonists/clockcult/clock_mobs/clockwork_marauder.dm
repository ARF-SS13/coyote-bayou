...
/mob/living/simple_animal/hostile/clockwork/marauder/guardian/proc/update_stats()
	if(GLOB.ratvar_awakens)
		...
		switch(healthpercent)
			...
			if(30 to 39) //Damage decrease, but not speed
				speed = initial(speed)
				melee_damage_lower = 10
				melee_damage_upper = 10
				attack_verb_continuous = "lightly slashes"
			if(20 to 29) //Speed decrease
				speed = 2
				melee_damage_lower = 8
				melee_damage_upper = 8
				attack_verb_continuous = "lightly slashes"
			if(10 to 19) //Massive speed decrease and weak melee attacks
				speed = 3
				melee_damage_lower = 6
				melee_damage_upper = 6
				attack_verb_continuous = "weakly slashes"
			if(0 to 9) //We are super weak and going to die
				speed = 4
