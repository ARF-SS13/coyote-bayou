...
/proc/voice_of_god(message, mob/living/user, list/span_list, base_multiplier = 1, include_speaker = FALSE, message_admins = TRUE)
	...
	//MOVE
		...
		for(var/iter in 1 to 5 * power_multiplier)
			for(var/V in listeners)
				var/mob/living/L = V
				addtimer(CALLBACK(GLOBAL_PROC_REF(_step), L, direction? direction : pick(GLOB.cardinals)), 10 * (iter - 1))
...
/proc/velvetspeech(message, mob/living/user, base_multiplier = 1, message_admins = FALSE, debug = FALSE)
	...
	if(findtext(message, enthrall_words))
		for(var/V in listeners)
			...
			if(E.lewd)
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, span_nicegreen("<i><b>[E.enthrallGender] is so nice to listen to.</b></i>")), 5)
			E.cooldown += 1
	...
	if(findtext(message, reward_words))
		for(var/V in listeners)
			...
			if(L == user)
				continue
			if (E.lewd)
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, span_love("[E.enthrallGender] has praised me!!")), 5)
				if(HAS_TRAIT(L, TRAIT_MASO))
					E.enthrallTally -= power_multiplier
					E.resistanceTally += power_multiplier
					E.cooldown += 1
			else
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, span_nicegreen("<b><i>I've been praised for doing a good job!</b></i>")), 5)
			E.resistanceTally -= power_multiplier
			E.enthrallTally += power_multiplier
		...
		for(var/V in listeners)
			...
			if (E.lewd)
				if(HAS_TRAIT(L, TRAIT_MASO))
					...
					E.enthrallTally += power_multiplier
					E.resistanceTally -= power_multiplier
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='love'>I've let [E.enthrallGender] down...!</b></span>"), 5)
				else
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, span_warning("I've let [E.enthrallGender] down...</b>")), 5)
			else
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, span_warning("I've failed [E.master]...</b>")), 5)
				E.resistanceTally += power_multiplier
				E.enthrallTally += power_multiplier
				E.cooldown += 1
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(0)
					...
					user.emote("snap")
					if(E.lewd)
						addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='big warning'>The snapping of your [E.enthrallGender]'s fingers brings you back to your enthralled state, obedient and ready to serve.</b></span>"), 5)
					else
						addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='big warning'>The snapping of [E.master]'s fingers brings you back to being under their influence.</b></span>"), 5)
	...
	//SILENCE
		...
		for(var/mob/living/carbon/C in listeners)
			...
			addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), C, "<span class='notice'>You are unable to speak!</b></span>"), 5)
	...
	//Antiresist
		...
		for(var/V in listeners)
			...
			var/datum/status_effect/chem/enthrall/E = L.has_status_effect(/datum/status_effect/chem/enthrall)
			E.status = "Antiresist"
			addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='big warning'>Your mind clouds over, as you find yourself unable to resist!</b></span>"), 5)
	...
	//RESIST
		...
		for(var/mob/living/carbon/C in listeners)
			...
			E.owner_resist()
			E.cooldown += 2
			addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), C, "<span class='notice'>You are spurred into resisting from [user]'s words!'</b></span>"), 5)
	...
	//FORGET (A way to cancel the process)
		...
		for(var/mob/living/carbon/C in listeners)
			var/datum/status_effect/chem/enthrall/E = C.has_status_effect(/datum/status_effect/chem/enthrall)
			if(E.phase == 4)
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), C, span_warning("You're unable to forget about [(E.lewd?"the dominating presence of [E.enthrallGender]":"[E.master]")]!</b>")), 5)
				continue
			addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), C, span_warning("You wake up, forgetting everything that just happened. You must've dozed off..? How embarassing!</b>")), 5)
			C.Sleeping(50)
			switch(E.phase)
				if(1 to 2)
					E.phase = -1
				...
				if(3)
					...
					E.cooldown = 0
					if(E.lewd)
						addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), C, "<span class='big warning'>You revert to yourself before being enthralled by your [E.enthrallGender], with no memory of what happened.</b></span>"), 5)
					else
						addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), C, "<span class='big warning'>You revert to who you were before, with no memory of what happened with [E.master].</b></span>"), 5)
	...
	//ATTRACT
		...
		for(var/V in listeners)
			...
			L.throw_at(get_step_towards(user,L), 3 * power_multiplier, 1 * power_multiplier)
			E.cooldown += 3
			addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You are drawn towards [user]!</b></span>"), 5)
		...
		for(var/mob/living/carbon/C in listeners)
			...
			switch(E.phase)
				if(2 to INFINITY)
					...
					E.cooldown += 10
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), C, "<span class='notice'>Drowsiness suddenly overwhelms you as you fall asleep!</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(2 to INFINITY)
					...
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), H, "<span class='[(E.lewd?"love":"warning")]'>Before you can even think about it, you quickly remove your clothes in response to [(E.lewd?"your [E.enthrallGender]'s command'":"[E.master]'s directive'")].</b></span>"), 5)
					E.cooldown += 10
			...
			switch(E.phase)
				if(2 to INFINITY)
					if(L.m_intent != MOVE_INTENT_WALK)
						L.toggle_move_intent()
						E.cooldown += 1
						addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You slow down to a walk.</b></span>"), 5)
			...
			switch(E.phase)
				if(2 to INFINITY)
					if(L.m_intent != MOVE_INTENT_RUN)
						L.toggle_move_intent()
						E.cooldown += 1
						addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You speed up into a jog!</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(2 to INFINITY)
					L.lay_down()
					E.cooldown += 10
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "[(E.lewd?"<span class='love'>You eagerly lie down!":"<span class='notice'>You suddenly lie down!")]</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(2 to INFINITY)
					...
					E.cooldown += 8
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You suddenly drop to the ground!</b></span>"), 5)
	...
	//STATE TRIGGERS
		...
		for(var/V in listeners)
			...
			if (E.phase == 3)
				...
				addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), C, span_notice("You whisper your triggers to [(E.lewd?"Your [E.enthrallGender]":"[E.master]")].")), 5)
				...
				if(!E.lewd)
					...
					if (E.mental_capacity >= 5)
						...
						if ((findtext(trigger2, custom_words_words)))
							...
							E.mental_capacity -= 5
							addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), H, span_notice("[(E.lewd?"your [E.enthrallGender]":"[E.master]")] whispers you a new trigger.")), 5)
			...
			if(E.phase == 3)
				if (get_dist(user, H) > 1)//Requires user to be next to their pet.
					...
					if (E.mental_capacity >= 200)
						...
						addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), H, span_notice("[(E.lewd?"Your [E.enthrallGender]":"[E.master]")] whispers you a new objective.")), 5)
		...
		for(var/V in listeners)
			...
			if(E.phase > 1)
				if(user.ckey == E.enthrallID && user.real_name == E.master.real_name)
					E.master = user
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), H, span_nicegreen("[(E.lewd?"You hear the words of your [E.enthrallGender] again!! They're back!!":"You recognise the voice of [E.master].")]</b>")), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3)//Tier 3 only
					...
					E.cooldown += 5
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You begin to lick your wounds.</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3 to INFINITY)
					...
					E.cooldown += 8
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>Your muscles freeze up!</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3 to INFINITY)
					...
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You feel your metabolism speed up!</b></span>"), 5)
	...
	//COLD
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3 to INFINITY)
					L.adjust_bodytemperature(-50 * power_multiplier)
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You feel your metabolism slow down!</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3 to INFINITY)//Tier 3 only
					...
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You jump to your feet from sheer willpower!</b></span>"), 5)
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(3)//Tier 3 only
					E.status = "pacify"
					E.cooldown += 10
					addtimer(CALLBACK(GLOBAL_PROC_REF(to_chat), L, "<span class='notice'>You feel like never hurting anyone ever again.</b></span>"), 5)
