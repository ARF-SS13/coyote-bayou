...
GLOBAL_LIST_INIT(slot2slot, list("head" = SLOT_HEAD, "wear_mask" = SLOT_MASK, "neck" = SLOT_NECK, "back" = SLOT_BACK, "wear_suit" = SLOT_WEAR_SUIT, "w_uniform" = SLOT_W_UNIFORM, "shoes" = SLOT_SHOES, "belt" = SLOT_BELT, "gloves" = SLOT_GLOVES, "glasses" = SLOT_GLASSES, "ears" = SLOT_EARS, "wear_id" = SLOT_WEAR_ID, "s_store" = SLOT_S_STORE))
...
/proc/changeling_transform(mob/living/carbon/human/user, datum/changelingprofile/chosen_prof)
	...
	for(var/slot in GLOB.slots)
		...
		C.inhand_icon_state = chosen_prof.item_state_list[slot]
		if(equip)
