...
/obj/item/hot_potato/proc/activate(delay, mob/user)
	...
	log_game("[user ? "[key_name(user)] primed [src]" : "A [src] has been primed"] ([detonate_dev_range]/[detonate_heavy_range]/[detonate_light_range]/[detonate_fire_range]) for detonation at [AREACOORD(T)]")
	active = TRUE
