...
/obj/item/artifact/proc/artifact_me()
	SSartifacts.artifactify(src, rareness)
	update_appearance()

/obj/item/artifact/proc/update_appearance()
	var/list/effects = list()
	SEND_SIGNAL(src, COMSIG_ITEM_ARTIFACT_GET_EFFECTS)
	if(!LAZYLEN(effects))
		return
	var/datum/artifact_effect/highest_effect
	var/highest_magnitude = 0 // pop pop
	for(var/datum/artifact_effect/AE in effects)
		var/magnitude = AE.get_magnitude()
		if(magnitude > highest_magnitude)
			highest_magnitude = magnitude
			highest_effect = AE
	if(istype(highest_effect, /datum/artifact_effect/passive_damage))
		var/datum/artifact_effect/passive_damage/PD = highest_effect
		PD.update_highest_damage()
		var/highest_damage = PD.highest_damage
		if(PD.is_buff)
