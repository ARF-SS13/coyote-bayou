	...
	if(alert(user,"Would you like to start the countdown now?",,"Yes","No") == "Yes" && in_range(src, user) && isliving(user))
		if(defused || active)
			...
			if(payload && !istype(payload, /obj/item/bombcore/training))
				message_admins("[ADMIN_LOOKUPFLW(user)] armed [name] ([payload]) at [ADMIN_VERBOSEJMP(bombturf)]</a>.")
...
/obj/item/bombcore/training
	name = "dummy payload"
	desc = "A US Government replica of a syndicate payload. Its not intended to explode but to announce that it WOULD have exploded, then rewire itself to allow for more training."
	var/defusals = 0
	var/attempts = 0
