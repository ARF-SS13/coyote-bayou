...
#define IS_PROCESSING(Processor, Datum) (Datum in Processor.processing)
#define START_PROCESSING(Processor, Datum) if (!IS_PROCESSING(Processor, Datum)) {Datum.datum_flags |= DF_ISPROCESSING;Processor.processing += Datum}
...
#define SS_ALWAYS_SHOW_STAT			(1<<6)
