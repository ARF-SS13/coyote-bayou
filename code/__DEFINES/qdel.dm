...
#define QDEL_IN(item, time) addtimer(CALLBACK(GLOBAL_PROC, .proc/______qdel_list_wrapper, (time) > GC_FILTER_QUEUE ? WEAKREF(item) : item), time)
#define QDEL_IN_STOPPABLE(item, time) addtimer(CALLBACK(GLOBAL_PROC, .proc/______qdel_list_wrapper, (time) > GC_FILTER_QUEUE ? WEAKREF(item) : item), time, TIMER_STOPPABLE)
#define QDEL_IN_CLIENT_TIME(item, time) addtimer(CALLBACK(GLOBAL_PROC, .proc/______qdel_list_wrapper, item), time, TIMER_STOPPABLE | TIMER_CLIENT_TIME)
