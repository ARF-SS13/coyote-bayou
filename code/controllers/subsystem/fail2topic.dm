...
/datum/controller/subsystem/fail2topic/proc/BanFromFirewall(ip)
	...
	if (.)
		log_admin("Failed to ban [ip]. Exit code: [.].")
		message_admins("Failed to ban [ip]. Exit code: [.].")
	else if (isnull(.))
		log_admin("Failed to invoke shell to ban [ip].")
		message_admins("Failed to invoke shell to ban [ip].")
	else
		log_admin("Banned [ip].")
		message_admins("Banned [ip].")
...
/datum/controller/subsystem/fail2topic/proc/DropFirewallRule()
	...
	if (.)
		log_admin("Failed to drop firewall rule. Exit code: [.].")
		message_admins("Failed to drop firewall rule. Exit code: [.].")
	else if (isnull(.))
		log_admin("Failed to invoke shell for firewall rule drop.")
		message_admins("Failed to invoke shell for firewall rule drop.")
	else
		log_admin("Firewall rule dropped.")
		message_admins("Firewall rule dropped.")
